...
1.1 High-level execution strategy
...
- Use SQLModel + SQLAlchemy engine creation compatible withPostgres.
...
- Create Better Auth instance (commonly in `frontend/lib/auth.ts`) and configure:
...
- Organize routes under `frontend/app/`:
  - Public routes:
    - `/sign-in`
    - `/sign-up`
  - Protected routes:
...
- Centralize API calls to FastAPI in `frontend/lib/api.ts` (or similar):
...
- Ensure `CLAUDE.md` remains the top-level Phase II instructions.
- Add/align:
  - `frontend/CLAUDE.md` with Next.js constraints and conventions.
  - `backend/CLAUDE.md` with FastAPI/SQLModel constraints and conventions.
