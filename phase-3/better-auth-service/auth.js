...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    customSession(async ({ user, session }) => {
      // Custom JWT claims to include standard fields
      // Return comprehensive user and session information with proper token
      return {
        user: {
          id: user.id,
          email: user.email,
          name: user.name,
          image: user.image, // Include user image if available
        },
        session: {
          ...session,
          token: session.token, // Ensure token is explicitly returned
          expiresAt: session.expiresAt, // Include expiration time
        }
      };
    })
  ],
